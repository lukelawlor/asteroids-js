...
function oWorldWrap() {
	if (this.x > GAME.WIDTH) this.x = GAME.WIDTH - this.x
	else if (this.x < 0) this.x = GAME.WIDTH + this.x

	if (this.y > GAME.HEIGHT) this.y = GAME.HEIGHT - this.y
	else if (this.y < 0) this.y = GAME.HEIGHT + this.y
}
...
function oCollideWithLarger(o, x1, y1, w1, h1, s1, s2, w2, h2) {
	...
	for (var i = 0; i < INSTANCES.MAX; ++i) {
		...
		if (other != null && other.o == o) {
			if (checkRect(x1, y1, w1, h1, other.x + s1, other.y + s2, w2, h2)) return other
		}
	}
	return null
}
...
function oCollideWithSmaller(o, x1, y1, w1, h1, s1, s2, w2, h2) {
	...
	for (var i = 0; i < INSTANCES.MAX; ++i) {
		other = inst[i]
		if (other != null && other.o == o) {
			if (checkRect(other.x + s1, other.y + s2, w2, h2, x1, y1, w1, h1)) return other
		}
	}
	return null
}
...
function drawBg() {
	...
	// Increase ticks
	if ((bxt1 += 0.3) > bw1) bxt1 = 0
	if ((byt1 += 0.1) > bh1) byt1 = 0

	if ((bxt2 += 0.2) > bw2) bxt2 = 0
	if ((byt2 += 0.05) > bh2) byt2 = 0
